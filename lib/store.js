...
export const db = {
  ...
  getLeadBySeoReportToken(token) {
    if (!token) return null;
    return load().leads.find((l) => l.seoReport?.token === token) || null;
  },

  getLeadByVoiceToken(token) {
    if (!token) return null;
    return load().leads.find((l) => l.voiceAgent?.token === token) || null;
  },
  ...
  upsertLeads(newLeads) {
    ...
    const PRESERVE = ['enrichment', 'siteId', 'status', 'notes', 'verification', 'outreach',
      ...
      'reviews', 'seoReport', 'voiceAgent', 'proof', 'capture', 'economics'];
