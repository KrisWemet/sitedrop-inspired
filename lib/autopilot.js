...
import { imageProviders, stockCandidates, attachStockImage, attachBundledImages } from './images.js';
import { mrr, retainersDue } from './billing.js';
...
export async function runCampaign(campaign, { log = true, searchFn = searchBusinesses, verifyFn = verifyLead } = {}) {
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      // 2. enrich (+ photos so the site is never empty). Prefer a licensed
      // stock photo (Pexels when configured, keyless Openverse CC0 otherwise);
      // fall back to the built-in industry set, which is instant and needs no
      // network — so an autopilot site always ships with images.
      const enrichment = await enrichLead(candidate);
      if (!(candidate.images || []).length) {
        let images = [];
        if (imageProviders().pexels || imageProviders().openverse) {
          try {
            const [first] = await stockCandidates(candidate, enrichment.industry, { perQuery: 1 });
            if (first) images = [await attachStockImage(candidate, first)];
          } catch { /* fall through to the built-in set */ }
        }
        if (!images.length) images = attachBundledImages(candidate, enrichment.industry);
        if (images.length) {
          db.updateLead(candidate.id, { images });
...
export function buildDigest() {
  ...
  // Recurring revenue: the money that makes the model near-passive.
  const clients = db.clients;
  const due = retainersDue(clients);
  if (due.length) {
    lines.push(`RETAINERS DUE (${due.length}) — issue this month's invoice with one click:`);
    for (const c of due.slice(0, 10)) lines.push(`  • ${c.businessName} — ${c.currency} ${c.monthly}/mo (since ${new Date(c.startedAt).toLocaleDateString()})`);
    lines.push('');
  }
  ...
  if (active.length) lines.push(`Clients: ${active.length} active · MRR ${active[0]?.currency || 'USD'} ${mrr(clients)}/mo`);
