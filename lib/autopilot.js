...
export async function runCampaign(campaign, { log = true, searchFn = searchBusinesses, verifyFn = verifyLead } = {}) {
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      // 2. enrich (+ one licensed stock photo: Pexels when configured,
      // keyless Openverse CC0 otherwise)
      const enrichment = await enrichLead(candidate);
      const stockAvailable = imageProviders().pexels || imageProviders().openverse;
      if (stockAvailable && !(candidate.images || []).length) {
        try {
