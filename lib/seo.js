...
// ---- placeholder contact detection ----
// Reserved example domains (RFC 2606) and common fake markers must never ship
// on a live site or in its schema — a real business email/site rendered as
// "dispatch@…example.com" is a delivery-day credibility bug.
const PLACEHOLDER_HOST = /(^|\.)(example\.(com|org|net)|test|invalid|localhost)$/i;

export function isPlaceholderEmail(email) {
  if (!email) return false;
  const at = String(email).split('@')[1];
  return at ? PLACEHOLDER_HOST.test(at.trim().toLowerCase()) : false;
}

export function isPlaceholderUrl(url) {
  if (!url) return false;
  try {
    return PLACEHOLDER_HOST.test(new URL(url.startsWith('http') ? url : 'https://' + url)
...
// Returns a shallow copy of the lead with placeholder email/website removed,
// plus a `placeholderContacts` list naming what was stripped (for operator UI).
export function sanitizeContacts(lead) {
  const stripped = [];
  const out = { ...lead };
  if (isPlaceholderEmail(lead.email)) { stripped.push(`email (${lead.email})`); out.email = null; }
  if (isPlaceholderUrl(lead.website)) { stripped.push(`website (${lead.website})`); out.website = null; out.hasWebsite = false; }
  out.placeholderContacts = stripped;
  return out;
}
...
export function buildFaqs(lead, profile) {
  ...
  if (profile.hoursHuman?.length) {
    faqs.push({
      ...
      a: `Our current hours are: ${profile.hoursHuman.join('; ')}. Hours may vary on holidays, so call ahead to confirm.`,
    });
  }
  ...
  const industryFaq = {
    ...
    health: { q: `Is ${name} accepting new patients?`, a: `Contact us to ask about new patient availability and insurance, and we'll get you scheduled as quickly as we can.` },
...
export function buildJsonLd(lead, profile, faqs) {
  ...
  // Review + AggregateRating are emitted ONLY from real operator-entered
  // reviews (never fabricated), computed straight from those entries. Self-
  // hosted review markup rarely earns a rich-result star in Google, but the
  // data is honest and useful to AI assistants; we never claim otherwise.
  if (Array.isArray(lead.reviews) && lead.reviews.length) {
    business.review = lead.reviews.map((r) => ({
      '@type': 'Review',
      author: { '@type': 'Person', name: r.author },
      reviewRating: { '@type': 'Rating', ratingValue: r.rating, bestRating: 5, worstRating: 1 },
      reviewBody: r.text,
    }));
    const sum = lead.reviews.reduce((s, r) => s + (Number(r.rating) || 0), 0);
    business.aggregateRating = {
      '@type': 'AggregateRating',
      ratingValue: (sum / lead.reviews.length).toFixed(1),
...
export function seoChecklist(lead, profile, { hasForm = false, hasBooking = false } = {}) {
  const hoursSpec = parseOpeningHours(lead.openingHours);
  return [
    ...
    { label: 'Lead capture: contact form or booking link', ok: hasForm || hasBooking },
    ...
    { label: 'Photography with alt text (client or licensed stock)', ok: Boolean((lead.images || []).length) },
  ];
}
