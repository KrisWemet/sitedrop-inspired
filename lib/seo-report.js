...
export function renderSeoReportHtml(lead, site, { pricing, period = new Date(), leads = null } = {}) {
...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .sub-h{font-size:.92rem;margin:18px 0 4px;color:var(--muted)}
...
<body>
  <div class="sheet">
    ...
    <main>
      ${leads ? `<section>
        <h2>What the site produced this month</h2>
        <div class="stat-row">
          <div class="stat"><b>${leads.total}</b><span>enquiries from your website${leads.changePct !== null ? ` · ${leads.changePct >= 0 ? '+' : ''}${leads.changePct}% vs last month` : ''}</span></div>
          <div class="stat"><b>${leads.calls}</b><span>calls to your tracked number</span></div>
          <div class="stat"><b>${leads.forms}</b><span>messages through the contact form</span></div>
        </div>
        ${leads.value ? `<p class="note"><strong>Estimated value: ${esc(formatMoney(leads.value.booked ?? leads.value.pipeline, pricing.currency))}${leads.value.booked === null ? ' of enquiry pipeline' : ' of booked work'}.</strong>
          ${leads.total} ${leads.total === 1 ? 'enquiry' : 'enquiries'} × ${esc(formatMoney(leads.value.avgJobValue, pricing.currency))} average job${leads.value.closeRate !== null ? ` × ${leads.value.closeRate}% close rate` : ''} — using the figures you gave us. The enquiry counts are logged events; the money is that arithmetic, not a measured result.</p>`
        : `<p class="note">Tell us your average job value and we will show what these enquiries are worth. The counts above are logged events, not estimates.</p>`}
        ${leads.recent.length ? `<h3 class="sub-h">Most recent</h3>
        <ul class="plain">
          ${leads.recent.slice(0, 5).map((e) => `<li class="ok">${esc(new Date(e.at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }))} — ${e.kind === 'call' ? 'Phone call' : `Website message${e.name ? ` from ${esc(e.name)}` : ''}`}</li>`).join('\n          ')}
        </ul>` : ''}
      </section>` : ''}
