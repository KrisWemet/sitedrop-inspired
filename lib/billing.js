...
export function pricingConfig() {
  return {
    ...
    // Premium package: the site plus the built upsells (voice agent + review
    // engine + SEO/AEO plan), positioned as the $5k tier.
    premium: int(process.env.PRICE_PREMIUM, 5000),
    premiumMonthly: int(process.env.PRICE_PREMIUM_MONTHLY, 500),
    // À-la-carte add-ons, so the operator can build any package from parts.
    addons: {
      voice: { setup: int(process.env.PRICE_VOICE_SETUP, 750), monthly: int(process.env.PRICE_VOICE_MONTHLY, 150) },
      reviews: { setup: int(process.env.PRICE_REVIEWS_SETUP, 300), monthly: int(process.env.PRICE_REVIEWS_MONTHLY, 75) },
      seo: { setup: int(process.env.PRICE_SEO_SETUP, 500), monthly: int(process.env.PRICE_SEO_MONTHLY, 300) },
    },
