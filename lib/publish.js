...
export function siteSlug(businessName, siteId, city = '') {
  const clean = (s) => String(s || '').toLowerCase()
    .replace(/&/g, ' and ').replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  const name = clean(businessName).slice(0, 50);
  const town = clean(city).slice(0, 20);
  // Short site-id suffix prevents two same-named businesses (e.g. "Main
  // Street Bakery" in different towns) from overwriting each other's project.
  const suffix = siteId.replace(/^site_/, '').slice(0, 4);
  return [name || 'site', town, suffix].filter(Boolean).join('-').slice(0, 90);
}

// Pure payload builder (unit-tested; the fetch wrapper below stays thin).
// imageFiles: [{ name, data: Buffer }] — shipped base64-encoded.
export function buildDeploymentPayload(site, html, imageFiles = []) {
  return {
    name: siteSlug(site.businessName, site.id, site.city),
    target: 'production',
    projectSettings: { framework: null },
    files: [
      { file: 'index.html', data: html, encoding: 'utf-8' },
      ...imageFiles.map((f) => ({ file: f.name, data: Buffer.from(f.data).toString('base64'), encoding: 'base64' })),
...
export async function publishSite(site, html, imageFiles = []) {
  if (!isPublishConfigured()) {
    throw new Error('Publishing is not configured. Set VERCEL_TOKEN (and optionally VERCEL_TEAM_ID) and restart.');
  }

  const payload = buildDeploymentPayload(site, html, imageFiles);
