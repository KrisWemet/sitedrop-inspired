...
// preview, download, or host anywhere. Every page ships with aggressive
// SEO/AEO: JSON-LD structured data, FAQ schema, geo meta tags, speakable
...
export function generateSite(lead, profile, themeKey) {
  ...
  const jsonLd = buildJsonLd(lead, profile, faqs);
...
<style>
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  .faq-list details{background:var(--surface);border:1px solid rgba(0,0,0,.07);border-radius:var(--radius);margin-bottom:12px;overflow:hidden}
  ...
  .faq-list details p{padding:0 22px 18px;color:var(--muted);font-size:.96rem}
...
<section class="hero" id="top" aria-label="Introduction">
  ${heroPattern()}
  <div class="wrap">
    <p class="kicker">${esc(titleCase(lead.category))}${cityLine ? ' · ' + esc(cityLine) : ''}</p>
    <h1>${name}${cityLine ? ` <span style="display:block;font-size:.45em;font-weight:400;opacity:.85;margin-top:10px">${esc(titleCase(lead.category))} in ${esc(cityLine)}</span>` : ''}</h1>
  ...
  <div class="wrap about-grid">
    <div>
      <div class="section-head">
        ...
        <h2 id="about-h">Welcome to ${name}</h2>
      </div>
      <p id="about-text" style="font-size:1.08rem">${esc(profile.about)}</p>
      ${cityLine ? `<p class="muted" style="margin-top:14px">Proudly serving ${esc(cityLine)} and surrounding communities. Looking for a trusted ${esc(lead.category)} near you? You just found one.</p>` : ''}
    </div>
...
<section id="services" style="padding-top:0" aria-labelledby="services-h">
  <div class="wrap">
    <div class="section-head">
...
<section id="faq" ${hours ? '' : 'style="padding-top:0"'} aria-labelledby="faq-h">
  <div class="wrap">
    <div class="section-head">
...
<section id="contact" style="padding-top:0">
  <div class="wrap">
    <div class="contact-band">
      <div>
        <p class="eyebrow" style="color:inherit;opacity:.8">Get in Touch</p>
...
<footer>
  <div class="wrap">
    <p>&copy; ${new Date().getFullYear()} ${name}. All rights reserved.${cityLine ? ` ${esc(titleCase(lead.category))} serving ${esc(cityLine)}.` : ''}</p>
  </div>
</footer>
</body>
</html>`;

  return { html, faqs, keywords, checklist, title: meta.title, description: meta.description };
}
