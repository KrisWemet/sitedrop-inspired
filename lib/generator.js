...
const THEMES = {
  warm: {
    name: 'Warm & Inviting',
    // Deliberately NOT the cream+terracotta default: wine and grain.
    bg: '#faf5ea', surface: '#fffdf6', ink: '#2a2118', muted: '#6f6353',
  ...
  bold: {
    ...
    oldstyleNums: false, dropCap: false, grain: false, cardHover: true, upperCta: true, cardAccent: true,
...
function heroInner(ctx) {
  ...
  // The kicker carries the local-SEO phrase; the h1 is the name alone.
  // Restating "{Type} in {City}" under the h1 was the loudest template tell.
  return {
...
function renderValues(ctx) {
  ...
  // Seed-vary the section rhythm: identical KICKER→H2 stacks six times in a
  // row is a template fingerprint, so some sites skip some eyebrows.
  return `<section class="values">
  <div class="wrap">
    <div class="section-head">
      ${ctx.seed % 2 === 0 ? `<p class="eyebrow">${esc(profile.headings.valuesEyebrow)}</p>` : ''}
...
function renderFaq(ctx) {
  ...
  <div class="wrap">
    <div class="section-head">
      ${ctx.seed % 3 !== 0 ? `<p class="eyebrow">${esc(ctx.profile.headings.faqEyebrow)}</p>` : ''}
...
${heroVariant === 'minimal' ? `
  ...
  ${t.rule ? '.section-head .eyebrow::after{content:"";display:block;width:48px;height:1px;background:var(--accent);margin-top:12px}' : ''}
  ...
  .card{background:var(--surface);border:1px solid rgba(0,0,0,.08);border-radius:var(--radius);padding:28px${t.cardHover ? ';transition:transform .15s ease' : ''}${t.cardAccent ? ';border-top:3px solid var(--accent)' : ''}}
...
${t.recipe.values.startsWith('statements') ? `
  ...
  ${t.recipe.values === 'statements-dash'
    ? '.val-stmt h3::before{content:"— ";color:var(--accent)}'
    : '.val-stmt{border-left:3px solid var(--accent);padding-left:20px}'}` : ''}
