...
import { buildMetaTags, buildJsonLd, buildFaqs, keywordSet, seoChecklist, sanitizeContacts } from './seo.js';
...
function heroInner(ctx) {
  ...
  return {
    ...
    // The primary CTA does what its label promises: a real booking link when
    // configured, otherwise it lands on the contact section (form/call) — never
    // a dead scroll to the services blurb.
    actions: `<div class="actions">
      ${lead.phone ? `<a class="btn" href="${telHref(lead.phone)}">Call ${esc(lead.phone)}</a>` : ''}
      ${ctx.bookingUrl
        ? `<a class="btn ghost" href="${esc(ctx.bookingUrl)}" target="_blank" rel="noopener">${esc(profile.heroCta)}</a>`
        : `<a class="btn ghost" href="#contact">${esc(profile.heroCta)}</a>`}
    </div>`,
  };
}
...
function ctaPanelAction(ctx) {
  const { lead } = ctx;
  if (ctx.bookingUrl) return `<a class="btn btn-invert" href="${esc(ctx.bookingUrl)}" target="_blank" rel="noopener">${esc(lead.enrichment?.heroCta || ctx.profile.heroCta)}</a>`;
  if (lead.phone) return `<a class="btn btn-invert" href="${telHref(lead.phone)}">Call ${esc(lead.phone)}</a>`;
  if (lead.email) return `<a class="btn btn-invert" href="mailto:${esc(lead.email)}">Email Us</a>`;
  return `<a class="btn btn-invert" href="#top">Back to Top</a>`;
}
...
function renderContact(ctx) {
  ...
  <div class="wrap">
    <div class="contact-band${hasForm ? ' contact-band-form' : ''}">
      <div>
      ...
      <div class="cta-panel">
        ${hasForm
          ? renderContactForm(ctx)
          : `<h3>${esc(ctx.bookingUrl ? profile.heroCta : (lead.phone
              ? ['Ready when you are', 'One call does it', "Let's talk today"][ctx.seed % 3]
              : profile.heroCta))}</h3>
        <p>${ctx.bookingUrl ? 'Book online in under a minute.' : (lead.phone ? 'One quick call is all it takes to get started.' : 'Reach out today, we respond fast.')}</p>
        ${ctaPanelAction(ctx)}`}
      </div>
    </div>
  </div>
