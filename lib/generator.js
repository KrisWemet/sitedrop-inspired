...
const THEMES = {
  ...
  bold: {
    ...
    oldstyleNums: false, dropCap: false, grain: false, cardHover: true, upperCta: true,
...
${heroVariant === 'minimal' ? `
  ...
  .card{background:var(--surface);border:1px solid rgba(0,0,0,.08);border-radius:var(--radius);padding:28px${t.cardHover ? ';transition:transform .15s ease' : ''}}
...
${t.recipe.values.startsWith('statements') ? `
  ...
  ${t.recipe.values === 'statements-dash'
    ...
    : '.val-stmt h3 span{background:linear-gradient(transparent 60%, color-mix(in srgb,var(--accent) 32%, transparent) 0)}'}` : ''}
