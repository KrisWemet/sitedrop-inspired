...
function renderCompactHero(ctx) {
  ...
  <div class="wrap">
    <div class="hero-inner">
      ...
      <p class="locale-line">${esc(titleCase(lead.category))}${ctx.cityLine ? ' in ' + esc(ctx.cityLine) : ''}</p>
      <div class="actions">
        ${lead.phone ? `<a class="btn" href="${telHref(lead.phone)}">Call ${esc(lead.phone)}</a>` : ''}
        ${heroSecondaryCta(ctx)}
      </div>
...
${compact ? `
  .hero-compact{background:${t.heroBg};color:${t.heroInk};padding:96px 0 104px}
  .hero-compact .hero-inner{max-width:660px}
  ...
  .hero-compact .hero-chips{margin:0 0 20px}
