...
function renderNav(ctx) {
  ...
  const navCta = lead.phone
    ...
    : ctx.bookingUrl
      ...
      : `<a class="btn" href="#contact">${ctx.formEndpoint ? esc(profile.heroCta) : 'Contact Us'}</a>`;
  return `<header>
  <div class="wrap nav">
...
function renderStickyCta(ctx) {
  ...
  if (!lead.phone && !ctx.bookingUrl && !ctx.formEndpoint) return '';
  ...
  const secondary = lead.phone && (ctx.bookingUrl || ctx.formEndpoint)
    ? (ctx.bookingUrl
...
function heroSecondaryCta(ctx) {
  ...
  if (ctx.formEndpoint) return `<a class="btn ghost" href="#contact">${esc(profile.heroCta)}</a>`;
...
// Three concrete deliverables, not adjectives. Sits directly under the hook so
// the value proposition is complete before the visitor has to scroll.
function renderPromiseList(ctx) {
  const promise = ctx.profile.promise || [];
  if (!promise.length) return '';
  return `<ul class="promise">${promise.map((p) => `<li>${icon('badge-check', { size: 18 })}<span>${esc(p)}</span></li>`).join('')}</ul>`;
}
...
function heroPrimaryCta(ctx) {
  ...
  if (ctx.formEndpoint || lead.email) return `<a class="btn btn-lg" href="#contact">${icon('mail', { size: 19 })}<span>${esc(profile.heroCta)}</span></a>`;
  return '';
}
...
function renderContactForm(ctx) {
  if (!ctx.formEndpoint) return '';
  return `<form class="lead-form" action="${esc(ctx.formEndpoint)}" method="POST">
...
function contactSub(ctx) {
  ...
  if (ctx.formEndpoint) return 'Send a message with the form and we will get right back to you.';
  if (lead.phone) {
...
function renderContact(ctx) {
  const { lead, profile } = ctx;
  const hasForm = Boolean(ctx.formEndpoint);
...
${t.recipe.values === 'checklist' ? `
  ...
  .promise{list-style:none;display:grid;gap:9px;margin:22px 0 28px;padding:0;max-width:34em}
  .promise li{display:flex;align-items:center;gap:10px;font-size:1rem;font-weight:500}
  .promise svg{flex:none;opacity:.85}
...
export function generateSite(rawLead, profile, themeKey, opts = {}) {
  ...
  const checklist = seoChecklist(lead, profile, { hasForm: Boolean(formEndpoint), hasBooking: Boolean(bookingUrl) });
